#!/usr/bin/env python3
"""Run the Strata suite and report line coverage of runner.py.

Uses only the standard library (`trace`), so it works on any Python 3.10+
without installing anything. `--min <percent>` fails the run below a threshold.
"""
...
TARGET = SRC_DIR / "strata" / "runner.py"


def executable_lines(path: Path) -> set[int]:
    """Every line the interpreter can stop on, taken from the compiled code objects.

    Lines marked `# pragma: no cover` are excluded; in this project that is only
    used for Windows-only branches and the `__main__` guard.
    """
...
def main() -> int:
    ...
    ap.add_argument("--min", type=float, default=0.0, help="minimum line coverage percent")
    ...
    print()
    print(f"coverage: runner.py {percent:.1f}%  "
          f"({len(executable) - len(missing)}/{len(executable)} executable lines)")
    if missing:
