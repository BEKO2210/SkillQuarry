...
:root[data-theme="light"] {
  ...
  --ink: #0c1821; --muted: #3d5b72; --dim: #546d85;
...
.card {
  ...
  border-radius: var(--radius); padding: 0 var(--s4) var(--s4);
...
/* The inset belongs to the card, not to its children: a child rule that resets
   its own margin can no longer push text against the edge. The picture is the
   one part that is meant to reach the edge, so it cancels the inset itself. */
.card { overflow: hidden; }
.card > .art { margin: 0 calc(-1 * var(--s4)); }
.card > .top { margin-top: var(--s4); }
.card > .foot { margin-bottom: 0; }
